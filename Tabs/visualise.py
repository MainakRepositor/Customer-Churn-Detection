...
def app(df, X, y):
    ...
    if st.checkbox("Tenure vs Churn"):
        sns.color_palette("rocket", as_cmap=True)
        ax=sns.scatterplot(x="tenure",y="Churn",data=df)
        st.pyplot()
    
    if st.checkbox("Monthly Charges vs Churn"):
        sns.color_palette("winter", as_cmap=True)
        ax=sns.scatterplot(x="MonthlyCharges",y="Churn",data=df)
        st.pyplot()

    if st.checkbox("Show Sample Results"):
        safe = (df['Churn'] == 0).sum()
        prone = (df['Churn'] == 1).sum()
        data = [safe,prone]
